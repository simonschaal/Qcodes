# **************************************************************************
#
# controller file for M4i.44x-x8
# this file use M4i digitizer to get the rerflectometry data
# **************************************************************************
#
# 
#
# Writen by: Luka Bavdaz, Marco Tagliaferri, Pieter Eendebak
# Modified by Jingyu Duan,  QSD @ UCL 
    ...
    def __init__(self, name, cardid='spcm0', **kwargs):
        ...
        self.add_parameter('temperature',
                           ...
                           get_cmd=partial(self._param32bit,
                                            pyspcm.SPC_MON_TC_BASE_CTRL), 
                           unit = 'Celsius',
                           docstring='The base card temperature') 
        ...
        self.add_parameter('end',
                           ...
                           unit='V',
                           vals=Numbers(-10, 10))
        self.add_parameter(name='acquisition1',
                           parameter_class=IQMagPhase)
        self.add_parameter(name='acquisition2',
                           parameter_class=RFtransport)
                           
        self.add_parameter('I_DC',
                           parameter_class=ManualParameter,
                           label='I DC offset for AC coupling',
                           initial_value=0,
                           vals=Numbers(-10000, 10000)) 
        ...
        self.add_parameter('input_path',
                           parameter_class=ManualParameter,
                           initial_value= 'buf' ,
                           label='input_path',
                           unit=None,
                           #val_mapping={'buf':0 , 'hf': 1},
                           vals = Enum('buf', 'hf'))
        ...
        self.add_parameter('impedance',
                           parameter_class=ManualParameter,
                           initial_value= 50,
                           label='Impedance (termination)',
                           unit=None,
                           #val_mapping={ '1M': 0, '50': 1},
        ...
        self.add_parameter('compensation',
                           ...
                           vals=Enum('enable', 'disable'),
                           docstring='if 1 enables compensation, if 0 disables compensation for channel')
        ...
        for i in [0, 1, 2, 3]:
            ...
            self.add_parameter('termination_{}'.format(i),
                               ...
                               set_cmd=partial(self._set_param32bit, getattr(
                                   pyspcm, 'SPC_50OHM{}'.format(i))),
                               val_mapping={ 1000000: 0, 50: 1},
                               vals = Enum( 1000000 , 50),
                               #vals=Enum(0, 1),
...
class IQMagPhase(MultiParameter):
    """
    Hardware pcontrolled parameter class for Alazar acquisition. To be used with
    ...
    def setup_sweep(self,channels):
        # -------
        # parameters to initilize the channels, has default values.
        input_range = self._instrument.input_range()
        input_path = self._instrument.input_path()
        input_coupling = self._instrument.input_coupling()
        impedance = self._instrument.impedance()
        compensation = self._instrument.compensation()
        level0 = 1500
        # --------------
        
        temp_size = int(self._instrument.sample_rate()/(2.*self._instrument.frequency()))
        posttrigger_size = temp_size + 16 - temp_size % 16
        pretrigger_size = 16
        memsize=  posttrigger_size + pretrigger_size
        seg_size = 1

        self._instrument.pretrigger_memory_size(pretrigger_size)
        self._instrument.posttrigger_memory_size(posttrigger_size)           
        self._instrument.initialize_channels(channels, input_range, input_path, impedance, input_coupling, compensation, memsize)
        self._instrument.set_ext0_OR_trigger_settings(pyspcm.SPC_TM_POS, impedance, input_coupling, level0)
        
        
        f = tuple(np.linspace(self._instrument.start(), self._instrument.end(), num=memsize))
        self.setpoints = ((f,), (f,), (f,), (f,))
        self.shapes = ((memsize,), (memsize,), (memsize,), (memsize,))
    ...
    def get(self):
        ...
        """
        mV_range = self._instrument.range_channel_0()
        nr_averages = self._instrument.averages()
        verbose = 0
        posttrigger_size = self._instrument.posttrigger_memory_size()
        buffered_data = self._instrument.blockavg_hardware_trigger_acquisition(mV_range, nr_averages,verbose, posttrigger_size)
        I = buffered_data[0::2]
        Q = buffered_data[1::2]
...
class RFtransport(MultiParameter):

    def __init__(self, name, instrument):
        super().__init__(name=name, names=('I', 'Q', 'magnitude', 'phase','X','Y','R'), shapes=( (),(),(),(),(),(),() ))
        self._instrument = instrument
        self.acquisition_kwargs = {}
        self.labels = ('I', 'Q', 'Magnitude', 'Phase','X','Y','R')
        #self.full_names = ('Magnitude', 'Phase', 'I', 'Q')
        self.units = ('V','V', 'V', 'Deg','V', 'V','V')
        self.setpoint_names = (('Sweep',), ('Sweep',), ('Sweep',), ('Sweep',), ('Sweep',), ('Sweep',),('Sweep',))
    ...
    def setup_sweep(self,channels):
        ...
        # self._instrument.enable_channels(pyspcm.CHANNEL0)
        # self._instrument.enable_channels(pyspcm.CHANNEL1)
        # self._instrument.enable_channels(pyspcm.CHANNEL2)
        # self._instrument.enable_channels(pyspcm.CHANNEL3)
        # self._instrument.set_channel_settings(0, input_range1, input_path, impedance, input_coupling, compensation)
        # self._instrument.set_channel_settings(1, input_range1, input_path, impedance, input_coupling, compensation)
        # self._instrument.set_channel_settings(2, input_range2, input_path, impedance, input_coupling, compensation)
        # self._instrument.set_channel_settings(3, input_range2, input_path, impedance, input_coupling, compensation)
        self._instrument.initialize_channels(channels, input_range, input_path, impedance, input_coupling, compensation,memsize)
        self._instrument.set_ext0_OR_trigger_settings(pyspcm.SPC_TM_POS, impedance, input_coupling, level0)
        

        f = tuple(np.linspace(self._instrument.start(), self._instrument.end(), num=memsize))
        self.setpoints = ((f,), (f,), (f,), (f,),(f,), (f,), (f,))
        self.shapes = ((memsize,), (memsize,), (memsize,), (memsize,),(memsize,), (memsize,),(memsize,))
    ...
    def get(self):
        ...
        recordB: numpy array of channel B acquisition
        """
        mV_range = self._instrument.range_channel_2()
        nr_averages = self._instrument.averages()
        verbose = 0
        posttrigger_size = self._instrument.posttrigger_memory_size()
